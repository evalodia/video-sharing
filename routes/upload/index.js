...
module.exports = async function (fastify, opts) {
  fastify.post('/', async function (request, reply) {
   const data = await request.file()
    ...
    const filename = path.join(__dirname, '../../public/videos/') + data.fields.name.value.replaceAll(' ','-') +
     '_' + 
     data.fields.league.value.replaceAll(' ','-') 
     + '_' 
     + data.fields.game.value.replaceAll(' ','-') + 
     '.mp4'
